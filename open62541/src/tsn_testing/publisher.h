...
UA_Int32 currentPublishCycleTime[MSG_COUNT+1];
struct timespec calculatedCycleStartTime[MSG_COUNT+1];
struct timespec cycleStartDelay[MSG_COUNT+1];
struct timespec cycleDuration[MSG_COUNT+1];
size_t publisherMeasurementsCounter  = 0;

static UA_UInt64 publishValue = 12345;
...
publishInterrupt(int sig, siginfo_t* si, void* uc) {
    ...
    if(publisherMeasurementsCounter >= MSG_COUNT)
        return;
    ...
    if(publisherMeasurementsCounter == MSG_COUNT) {
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                    "Logging the measurements to %s", output_file);

        FILE *fpPublisher = fopen(output_file, "w");
...
static void
addPubSubConfiguration(UA_Server* server, int interval) {
    ...
    connectionConfig.name = UA_STRING("TSN UDP-UADP Connection");
...
int run(int interval) {
    ...
    addPubSubConfiguration(server, interval);
