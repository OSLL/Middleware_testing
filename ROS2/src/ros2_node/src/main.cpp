...
int main(int argc, char **argv) {
    ...
    try {
        if (type_name == "publisher"){
            ...
            Publisher pub(topic1, m_count, priority_pub, cpu_index_pub, min_msg_size, max_msg_size, step, interval,
                          msgs_before_step, filename_pub, topic_prior);
            pub.StartTest();
        }
        else if (type_name == "subscriber") {
            Subscriber sub(topic1, m_count, priority_sub, cpu_index_sub, filename_sub, topic_prior);
            sub.StartTest();
        }
