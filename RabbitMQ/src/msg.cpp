...
void Message::get_bytes(std::string& bytes){
	...
	*((int32_t*)(bytes.c_str()))=id;
	*((uint64_t*)(bytes.c_str()+sizeof(int32_t)+sizeof(uint64_t)))=timestamp;
	memcpy((void*)(bytes.c_str()+sizeof(int32_t)+sizeof(uint64_t)),str.c_str(),str.length());
}

void Message::set_from_bytes(std::string& bytes){
	id=*((int32_t*)(bytes.c_str()));
	timestamp=*((int32_t*)(bytes.c_str()+sizeof(int32_t)));
