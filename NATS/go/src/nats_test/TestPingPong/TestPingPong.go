...
func New(topic1 string, topic2 string, msgCount int, prior int, cpu_index int, msgSize int, interval int, filename string, topic_priority int, isFirst bool) TestPingPong{
	...
	pingpong := TestPingPong{topic1, topic2, msgCount, prior, cpu_index, msgSize, interval, filename, topic_priority, isFirst, nc, make([][]byte, msgCount, msgCount), make([]int64, msgCount, msgCount), new(int), new(int)}
